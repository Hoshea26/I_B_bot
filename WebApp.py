...
import random
import numpy as np
from contract_obj import UnderlyingContract, OptionContract
...
class MarketDataApp(EWrapper, EClient):
    ...
    def historicalData(self, reqId, bar):
        global put_bid_hist_df
        if reqId == 1:
            hist_dict = {"open": bar.open, "high": bar.high, "low": bar.low, "close": bar.close}
            time.sleep(1)
            new_row = pd.DataFrame.from_dict(hist_dict, orient="index", columns=[bar.date]).T
            put_bid_hist_df = put_bid_hist_df._append(new_row)
...
def main():
    ...
    app_thread.start()
    app.reqMktData(1, underlying_contract.contract, "", False, False, [])
    ...
    call_contract = OptionContract(option_e_date, call_strike, "CALL")
    put_contract = OptionContract(option_e_date, put_strike, "PUT")

    # Request market data for the specified contract
    app.reqMktData(2, call_contract.contract, "", False, False, [])
    app.reqMktData(3, put_contract.contract, "", False, False, [])
    app.reqHistoricalData(reqId=1,
                          contract=put_contract.contract,
                          endDateTime="",
                          durationStr="600 s",
...
def update_put_bid_graph(n):
    trace_hist_candle = go.Candlestick(x=list(put_bid_hist_df.index),
